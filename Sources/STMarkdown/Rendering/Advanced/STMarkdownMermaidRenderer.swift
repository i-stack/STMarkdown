...
public class STMarkdownMermaidRenderer: NSObject {
    ...
    public func cachedImage(for code: String, theme: STMarkdownThemeMode) -> UIImage? {
        self.imageCacheStore.object(forKey: self.cacheKey(code, theme) as NSString)
    }

    /// 异步渲染 Mermaid 图表。若已缓存立即回调；否则入队等待 WKWebView 渲染完成后回调。
    /// 回调闭包在主线程触发；标注 `@Sendable` 是为兼容 Swift6 严格并发调用方。
    public func renderAsync(code: String, width: CGFloat, theme: STMarkdownThemeMode, completion: @escaping @Sendable (UIImage?) -> Void) {
        let key = self.cacheKey(code, theme)
    ...
    func cacheKey(_ code: String, _ theme: STMarkdownThemeMode) -> String {
        "\(theme.isDark ? "1" : "0")_\(code)"
    }
    ...
    private func drainQueue() {
        ...
        self.renderOne(next.code, width: next.width, theme: next.theme, key: next.key)
    }

    private func renderOne(_ code: String, width: CGFloat, theme: STMarkdownThemeMode, key: String) {
        ...
        let encodedTheme = Self.jsStringLiteral(for: themeName)
